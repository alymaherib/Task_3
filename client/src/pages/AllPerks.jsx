...
export default function AllPerks() {
  ...
  // Build unique merchant list any time perks change
  useEffect(() => {
  ...
  async function loadAllPerks() {
    ...
    try {
      ...
      // Accept either { perks: [...] } or raw array
      const list = Array.isArray(res.data) ? res.data : res.data?.perks || []
      setPerks(list)
    } catch (err) {
  ...
  function handleSearch(e) {
    e.preventDefault()
    // Immediate fetch (bypasses debounce) when user presses Enter / clicks button
    loadAllPerks()
  }
  ...
  function handleReset() {
    setSearchQuery('')
    setMerchantFilter('')
    // The auto-effect will trigger and reload all
  }
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {perks.map((perk) => (
          <Link
            key={perk._id}
            to="#"
            /* If you have a details route, change to={`/perks/${perk._id}`} */
